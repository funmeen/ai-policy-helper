...
def ask(req: AskRequest):
    ...
    answer, contexts = engine.generate(req.query, ctx)
    citations = [Citation(title=c.get("title"), section=c.get("section")) for c in contexts]
    chunks = [Chunk(title=c.get("title"), section=c.get("section"), text=c.get("text")) for c in contexts]
    stats = engine.stats()
    return AskResponse(
        query=req.query,
        answer=answer,
